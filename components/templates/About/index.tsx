...
import {imgProfile, infoBiography, skills} from "../../../config";
                    ...
                    <div className={'container-info'}>
                        ...
                        <div className={'row'}>
                            {infoBiography.map((item, i) => (
                                <div className={'col-sm-6 my-2 d-flex align-items-center'}>
                                    <Text fontWeight={'500'} className={'text-capitalize mr-3'}>{item.name} </Text>
                        ...
                        <div className={'row'}>
                            {skills.map((item, i) => (
                                <DonutChartSkill delay={i * 0.5} key={item.name} percentage={item.percentage}
                                                 className={'col-6 col-sm-4 col-md-3 my-2'} name={item.name}
                                                 icon={<Icon className={'icon'} name={item.name}/>}/>
                            ))}
                        </div>
                        <Text> jupsu clones de sombra, picar diamante en minecraft, controlar carteles de naruto </Text>
                        <Title className={'my-2'} tagTitle={'h2'} fontSize={'1.7em'} fontWeight={'600'}
